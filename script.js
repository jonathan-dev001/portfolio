  /* ================================================================
       JS — PORTFOLIO JONATHAN HERVÉ
       ================================================================
       SECTIONS :
       01. Thème clair/sombre
       02. Sécurité (email obfusqué + FormSubmit)
       03. Navigation (burger, scroll, liens actifs)
       04. Langue FR/EN
       05. Animations scroll (fadeUp + compteur)
       06. Stack bars
       07. Budget slider
       08. Formulaire (validation + envoi)
    ================================================================ */

    // ---- 01. THÈME — déclaré en premier pour être disponible partout ----
    (function() {
      ...
      window.applyTheme = function(mode) {
        ...
        if (themeBtn) themeBtn.setAttribute('aria-label', isLight ? 'Passer en thème sombre' : 'Passer en thème clair');
      ...
      if (saved) {
        ...
        window.applyTheme(window.matchMedia('(prefers-color-scheme: light)').matches ? 'light' : 'dark');
      }
      ...
      if (themeBtn) {
        themeBtn.addEventListener('click', () => {
          window.applyTheme(document.documentElement.classList.contains('light') ? 'dark' : 'light');
        });
      }
    })();
    ...
    (function() {
      // Email reconstruit depuis des morceaux — invisible aux scrapers basiques
      const u = ['jonathan','herve0502'].join('');
      const d = ['gmail','.com'].join('');
      const em = u + '@' + d;

      // Lien email carte contact
      ...
      // FormSubmit action injectée par JS (hash non visible dans le HTML source)
      const form = document.getElementById('fsForm');
      if (form) {
        const h = ['63dcccd0f341','5832','761d8b3c','6851b744'].join('');
        form.action = 'https://formsubmit.co/' + h;
        // Champs cachés ajoutés dynamiquement
        const fields = {
          '_captcha': 'false',
          '_template': 'table',
          '_subject': 'Nouveau message depuis le portfolio !',
          '_next': window.location.origin + window.location.pathname + '?success=1#contact',
          '_autoresponse': 'Merci pour votre message ! Je vous répondrai dans les plus brefs délais.',
        };
        ...
        honey.type = 'text';
        honey.name = '_honey';
        honey.style.cssText = 'display:none;position:absolute;left:-9999px';
        honey.tabIndex = -1;
    ...
    // Toggle thème depuis le bouton mobile
    ...
    const links = document.querySelectorAll('.nl a[data-s], .mobile-menu a[data-s]');
    const ids   = ['accueil','a-propos','services','projets','stack','temoignages','contact'];
    ...
    function majNav() {
      ...
      ids.forEach(id => {
        const el = document.getElementById(id);
        if (!el) return;
        if (el.getBoundingClientRect().top <= window.innerHeight * .38) current = id;
      });
    ...
    // ---- 04. LANGUE : innerHTML pour les éléments avec balises HTML ----
    let lang = 'fr';
    const titles = { fr: 'Jonathan Hervé — Développeur Full-Stack à Montréal', en: 'Jonathan Hervé — Full-Stack Developer in Montreal' };
    document.querySelectorAll('.lb').forEach(btn => {
      btn.addEventListener('click', () => {
        lang = btn.dataset.l;
        document.documentElement.lang = lang;
        document.title = titles[lang];
        document.querySelectorAll('.lb').forEach(b => b.classList.toggle('on', b === btn));
        document.querySelectorAll('[data-fr]').forEach(el => {
          ...
          if (el.tagName === 'INPUT' || el.tagName === 'TEXTAREA') {
            ...
            // Contient du HTML (ex: <strong>) → innerHTML
            el.innerHTML = value;
          } else {
    ...
    document.querySelectorAll('.card.fact').forEach(el => countObserver.observe(el));
    ...
    const stackObserver = new IntersectionObserver(entries => {
      entries.forEach(entry => {
        ...
        if (fill) fill.style.width = (parseFloat(entry.target.dataset.w) * 100) + '%';
        stackObserver.unobserve(entry.target);
      });
    }, { threshold: .3 });
    document.querySelectorAll('.stack-card').forEach(el => stackObserver.observe(el));
    ...
    (function() {
      const steps = [
        ...
        { val: '2 000 $',    tagFr: 'Site vitrine avancé',  tagEn: 'Advanced showcase' },
      ...
      // Re-sync au changement de langue
      document.querySelectorAll('.lb').forEach(btn => {
    ...
    (function() {
      ...
      form.querySelectorAll('.cf-input, .cf-select, .cf-textarea').forEach(inp => {
        inp.addEventListener('blur', () => validate(inp));
        inp.addEventListener('input', () => {
          if (inp.closest('.cf-group').classList.contains('has-error')) validate(inp);
        });
      });
      ...
      function validate(inp) {
        ...
        if (inp.type === 'email' && inp.value.trim() && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(inp.value)) ok = false;
        if (inp.tagName === 'TEXTAREA' && inp.hasAttribute('minlength') && inp.value.trim().length < parseInt(inp.getAttribute('minlength'))) ok = false;
      ...
      form.addEventListener('submit', function(e) {
        // Rate limiting — pas deux soumissions en moins de 8s
        const now = Date.now();
      ...
      // Confirmation si redirection FormSubmit
